...
подсветка скачет — поможет более крупная модель (`ASR_MODEL=medium`/`large-v3`)
и правильный язык. Для текста в деванагари обычно лучше `hi`, для романизации —
тоже `hi` (нормализация снимает диакритику).
