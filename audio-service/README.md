...
При создании задачи можно передать **текст песни** (`lyrics`) и **язык**
(`language`: `auto`/`ru`/`en`/`bn`/`hi`/`sa`/…) прямо в форме UI или в API.
Язык задачи имеет приоритет над `ASR_LANGUAGE` из `.env`; `auto` — Whisper
определит сам. Для быстрой проверки пайплайна удобно взять простую песню на
`ru`/`en` — там нет ни хора, ни кросс-скрипта. Тогда сервис:

1. Прогоняет `vocals.wav` через `faster-whisper` на языке `ASR_LANGUAGE`,
   получая сегменты со start/end **в порядке исполнения**.
2. Каждый сегмент сопоставляется с самой похожей строкой текста
   (`difflib.SequenceMatcher` по нормализованной форме). **Whisper-у не доверяем
   как тексту** — только времени. Получается **таймлайн** с повторами: строка,
   спетая N раз, подсветится N раз (киртаны почти всегда повторяют мантру).
3. **Текст известен целиком**, поэтому строки, которые Whisper не распознал, не
   теряются: их тайминги интерполируются между распознанными «якорями» (помечаются
   `interpolated=true`). Так подсветка и аккорды есть у каждой строки.
4. Манифест отдаёт `lyrics_lines` (текст для показа, с таймингом и аккордами) и
   `lyrics_timeline` (`[{start, end, line, chords}]` — ведёт подсветку, включая
   интерполированные строки). Всё сохраняется в `lyrics.json`.

**Кросс-скрипт.** Текст часто дают романизацией (IAST: «āmāra jīvana»), а Whisper
распознаёт в деванагари/бенгали. Поэтому перед сравнением обе стороны приводятся
к латинице через `indic-transliteration` (вывод Whisper → IAST → без диакритики).
Без этой библиотеки латиница и индийское письмо не совпадут и выравнивания не
будет — поставьте `indic-transliteration`.

Подводные камни киртана: хор/перекличка и наложенные голоса ухудшают ASR. Если
подсветка скачет или пустая — проверьте:
- **`ASR_LANGUAGE`** соответствует песне (для бенгальских бхаджанов — `bn`);
- установлен **`indic-transliteration`** (если текст латиницей);
- более крупная модель **`ASR_MODEL=medium`/`large-v3`** устойчивее к хору.
...
аудио по таймлайну (с повторами), клик по строке перематывает на её первое
появление. Строки без привязки показываются приглушённо.

**Аккорды над словами.** Слова строки расставляются по времени с использованием
**пословных таймстампов Whisper** (учитывают паузы и распевы; если их нет —
запасной вариант по длине слов), а смена аккорда ставится над тем словом, где она
реально происходит (тайминги аккордов — с дорожки фисгармони). Получается раскладка
как в гитарных табах. Данные — в `lyrics_lines[i].words = [{text, chords}]`.

Аккордовая дорожка строится «как у музыканта». Источник по умолчанию —
**стем вокала** (`CHORD_SOURCE=vocals`): мелодия чище задаёт тональность и
подразумеваемую гармонию, чем шумный стем фисгармони. Оценивается **тональность**
(`result.key`, напр. `Bm`), звук делится по **долям** (beat tracking), доли
группируются в **такты** (`CHORD_BEATS_PER_BAR=4`) — аккорд берётся на такт, а не
на каждую долю (иначе к концу трека «вакханалия» из десятков смен). На каждом
такте по усреднённой хроме подбирается трезвучие со смещением в сторону диатоники
тональности (убирает дрожание maj↔min). Последовательность сглаживается
**алгоритмом Витерби** со штрафом за смену аккорда (`CHORD_CHANGE_PENALTY`, больше
→ меньше смен) — держит аккорд, пока нет сильных оснований сменить, и убирает
«вакханалию» из сотен смен. Соседние одинаковые и короткие аккорды сливаются
(`CHORD_MIN_DURATION`).

См. `PRODUCT.md` — видение продукта и план по этапам (ядро — редактируемый
публичный аккордовый лист, схемы аккордов для инструментов, доведённый MIDI;
караоке — сайд-проект).
