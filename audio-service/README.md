...
Аккордовая дорожка строится «как у музыканта»: оценивается **тональность** песни,
звук делится по **долям** (beat tracking), на каждом отрезке берётся усреднённая
хрома и подбирается трезвучие со смещением в сторону диатоники тональности (это
убирает дрожание maj↔min на дроне фисгармони). Соседние одинаковые и короткие
аккорды сливаются (`CHORD_MIN_DURATION`) → короткая честная последовательность
≈ аккорд на такт. Тональность отдаётся в `result.key` (напр. `Bm`).
