...
3. Манифест отдаёт `lyrics_lines` (текст для показа, с таймингом первого
   появления и аккордами) и `lyrics_timeline` (`[{start, end, line, chords}]` —
   ведёт подсветку). Всё сохраняется в `lyrics.json`.
