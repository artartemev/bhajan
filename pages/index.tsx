// File: pages/index.tsx (финальная версия с исправленной JSX-разметкой)
...
// ✅ ИСПРАВЛЕНИЕ: Исправлены синтаксические ошибки в JSX
function LessonsScreen() {
    const navigate = useNavigate();
    const { id } = useParams<{ id: string }>();
    const { data: bhajan } = useQuery(["bhajan", id], () => apiClient.getBhajanDetail({ id: id! }), { enabled: !!id });
    if (!bhajan) return <div>Loading...</div>;
    if (!bhajan.lessonsUrl) return <div className="p-4">No lessons available. <Button onClick={() => navigate(-1)}>Back</Button></div>;
            ...
            <header className="sticky top-0 bg-background/95 backdrop-blur-sm border-b border-border p-4 zen-shadow">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        <h1 className="text-xl font-medium zen-heading">Lessons</h1>
...
function FavoritesScreen() {
    ...
    return (
        <div className="pb-32">
            <header className="sticky top-0 bg-background/95 backdrop-blur-sm border-b border-border p-4 zen-shadow">
                <h1 className="text-2xl font-light zen-heading text-center">Favorites</h1>
            </header>
            <div className="p-4 space-y-3">
