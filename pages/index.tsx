// File: pages/index.tsx (финальная версия с исправленной загрузкой данных)
...
function PianoChordDiagram({ notes, description }: { notes: string; description: string; }) { const keys = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]; const pressedNotes = notes.split("-").map(n => n.replace('b', '#')); return (<div className="p-3 bg-card rounded-md border"><p className="text-center font-bold text-sm mb-2">{description}</p><div className="flex justify-center">{keys.map(key => <PianoKey key={key} isBlack={key.includes("#")} isPressed={pressedNotes.includes(key)} label={key} />)}</div></div>); }
